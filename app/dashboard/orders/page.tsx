            ...
            <table className="hidden min-w-full text-gray-900 md:table">
              <thead className="rounded-lg text-left text-sm font-normal">
                <tr>
                  ...
                  <th scope="col" className="px-3 py-5 font-medium">Supplier</th>
                  <th scope="col" className="px-3 py-5 font-medium">Orders</th>
                  <th scope="col" className="px-3 py-5 font-medium">Subtotal</th>
                  <th scope="col" className="px-3 py-5 font-medium">IVA</th>
              ...
              <tbody className="bg-white">
                {invoices.map((invoice) => (
                  <tr key={invoice.id} className="w-full border-b py-3 text-sm last-of-type:border-none [&:first-child>td:first-child]:rounded-tl-lg [&:first-child>td:last-child]:rounded-tr-lg [&:last-child>td:first-child]:rounded-bl-lg [&:last-child>td:last-child]:rounded-br-lg">
                    <td className="whitespace-nowrap py-3 pl-6 pr-3">{invoice.id}</td>
                    <td className="whitespace-nowrap px-3 py-3">{invoice.proveedorNombre}</td>
                    <td className="whitespace-nowrap px-3 py-3">{invoice.pedidos.length} orders</td>
                    <td className="whitespace-nowrap px-3 py-3">Q. {invoice.subtotal.toFixed(2)}</td>
                    <td className="whitespace-nowrap px-3 py-3">Q. {invoice.iva.toFixed(2)}</td>
                    <td className="whitespace-nowrap px-3 py-3 font-medium">Q. {invoice.totalFactura.toFixed(2)}</td>
